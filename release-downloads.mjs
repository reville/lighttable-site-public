import { releaseManifest } from './release-data.mjs?release=566e421992eedc4d';
...
export function manifestDownload(platform, architecture, manifest = releaseManifest) {
  ...
  const preferredMacDmg = platform === 'macos' && entry.signing === 'developer-id-notarized'
    ? entry.artifacts.filter(artifact => artifact.name.endsWith('-macos-arm64-notarized.dmg')) : [];
  const candidates = preferredMacDmg.length ? preferredMacDmg : entry.artifacts.filter(artifact => artifact.name.endsWith(endings[platform]));
